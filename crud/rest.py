"""
Simple REST API for CRUD operations, when identity is string representation of uuid, generated by PyMotyc 
and is part of model, which is single one and used both on database and network level.

Using PyMotyc generated uuid as resource id is very convenient, and much much more easier then to rely
...
async def create_employee(employee: Employee) -> Employee:
    ...
    if employee.id:
        raise HTTPException(status_code=400, detail="Can't create resource, when id is given, use w/o id.")
...
async def put_employee(_id: str, employee: Employee) -> Employee:
    """ Updates employee completely in database by given id, id itself can not be modified and can be omitted."""
    if employee.id is not None and employee.id != _id:
        raise HTTPException(status_code=400, detail="Can't put resource, id can not be modified.")
    employee.id = _id
...
if __name__ == "__main__":
    unknown_id = Warehouse.employees.generate_id()
    ...
    with TestClient(app) as cli:
        ...
        vasya = response.json()
        vasya_id = vasya['id']
        assert uuid.UUID(vasya_id)  # Vasya's id is str representation of uuid, generated by pymotyc.
        ...
        response = cli.post('/employees', json={'id': unknown_id, 'full_name': 'Dusya Ivanova', 'age': 20})
        assert response.status_code == 400  # can't create with id
        ...
        response = cli.put('/employees/' + vasya_id, json={'id': vasya_id, 'full_name': 'Vasya Pupkin', 'age': 43})
        assert response.status_code == 200  # ok, ids are the same in resource and url

        response = cli.put('/employees/' + vasya_id, json={'id': unknown_id, 'full_name': 'Vasya Pupkin', 'age': 43})
        assert response.status_code == 400  # not ok, ids are not the same in resource and url

        response = cli.get('/employees/' + vasya_id)
        assert response.status_code == 200
        assert response.json() == {'id': vasya_id, 'full_name': 'Vasya Pupkin', 'age': 43}

        response = cli.get('/employees/' + unknown_id)
        assert response.status_code == 404

        response = cli.put('/employees/' + unknown_id, json={'full_name': 'Dusya Ivanova', 'age': 20})
        assert response.status_code == 404
        ...
        response = cli.patch('/employees/' + unknown_id + '?inc_age=2')
        assert response.status_code == 404
