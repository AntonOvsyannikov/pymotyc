...
and all those points are out of scope of pymotic, which only manages app<->database communication,
while serialization of the model for network communication is not part of it, so we do it by hands,
converting database model to network one.

For that reason, it is much much easier to use str representation of uuid, generated by pymotic as resource id,
which is covered in rest3.py and is a recommended way to use pymotyc.
...
class Employee(EmployeeIn, pymotyc.MotycModel):
    pass
